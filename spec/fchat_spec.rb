...
describe Bottlebot do
  before :each do
    @bot_desc = 'Bottlebot testbot by Jippen Faddoul (https://github.com/jippen/fchat_bottlebot_ruby)'
    @bot_version = '1.0'
    @fchat = Bottlebot.new(@bot_desc, @bot_version)
    @fchat.logger.level = Logger::FATAL
  end

  it "takes no parameters and returns a Bottlebot object" do
    @fchat.should be_an_instance_of Bottlebot
  end
